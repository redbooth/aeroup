...
user = Table('user', post_meta,
    ...
    Column('oauth_token', Unicode(length=256)),
    Column('first_name', Unicode(length=256)),
    Column('last_name', Unicode(length=256)),
)
...
def upgrade(migrate_engine):
    ...
    post_meta.tables['user'].columns['oauth_token'].create()
...
def downgrade(migrate_engine):
    ...
    post_meta.tables['user'].columns['oauth_token'].drop()
